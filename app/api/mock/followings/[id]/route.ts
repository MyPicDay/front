...
export async function GET(
  ...
  // 이 사용자가 팔로우하는 사람들(내가 팔로우한 대상)
  const myFollowings = followers.filter((f: any) => f.followerId === id);

  // 사용자 정보와 isFollowing(내가 팔로우 중인지) 포함
  const result = myFollowings.map((f: any) => {
    const user = users.find((u: any) => u.id === f.followingId);
    return {
      id: user.id,
      name: user.name,
      avatar: user.avatar,
